...
public class PagamentoController {
    ...
    public String listar(@RequestParam(value = "editar", required = false) Long editarId, Model model) {
        ...
        carregarListasTela(model, pagamento);
    ...
    private void carregarListasTela(Model model, Pagamento pagamento) {
        List<LancamentoResumoDto> lancamentosDisponiveis = service.listarResumoLancamentos().stream()
                .filter(item -> item.saldoAberto().compareTo(BigDecimal.ZERO) > 0
                        || (pagamento.getLancamento() != null
                        && pagamento.getLancamento().getId() != null
                        && item.lancamento().getId().equals(pagamento.getLancamento().getId())))
                .toList();

        model.addAttribute("lancamentosDisponiveis", lancamentosDisponiveis);
