...
public class CondominioService {
    ...
    public RelatorioMensalResumoDto resumirRelatorioMensal(List<RelatorioUnidadeDto> itens) {
        ...
        BigDecimal totalPago = itens.stream()
                .map(RelatorioUnidadeDto::totalPago)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal saldoAberto = itens.stream()
                .map(RelatorioUnidadeDto::saldoAberto)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        return new RelatorioMensalResumoDto(totalLancado, totalPago, saldoAberto);
    }
